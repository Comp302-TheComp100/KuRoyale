 ...
 * Uses GRASP Information Expert - knows how to load and provide its own
 * settings.
 */
public class NetworkConfig {
    ...
    private void loadFromStream(InputStream is) throws IOException {
        ...
        this.connectionTimeout = parseIntWithDefault(props, "CONNECTION_TIMEOUT", DEFAULT_CONNECTION_TIMEOUT, 1000,
                30000);
        ...
        this.heartbeatInterval = parseIntWithDefault(props, "HEARTBEAT_INTERVAL", DEFAULT_HEARTBEAT_INTERVAL, 500,
                5000);
    }
    ...
    private int parseIntWithDefault(Properties props, String key, int defaultValue, int min, int max) {
        ...
        try {
            ...
            if (parsed < min || parsed > max) {
                System.err.println("[NetworkConfig] " + key + " out of range (" + min + "-" + max + "), using default: "
                        + defaultValue);
                return defaultValue;
            }
            return parsed;
