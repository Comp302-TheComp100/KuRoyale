...
public class NetworkBattleController implements GameEventListener {
    ...
    private void handleFullStateSync(NetworkMessage message) {
        ...
        try {
            ...
            // Apply troop state with smart updates (updates existing troops instead of recreating)
            try {
            ...
            // Apply building state with smart updates (updates existing buildings instead of recreating)
            try {
    ...
    private void applyTroopState(String troopData) {
        ...
        for (String troopStr : troops) {
            ...
            try {
                ...
                // Try to find an existing troop to update (match by card name, side, and proximity)
                Troop matchedTroop = findMatchingTroop(existingTroops, matchedTroops, cardName, clientIsPlayerSide, clientWorldX, clientWorldY);
    ...
    private Troop findMatchingTroop(java.util.List<Troop> existingTroops, java.util.Set<Troop> alreadyMatched, 
        ...
        // Larger matching distance for swarm troops to handle multiple units of same type
        double maxDistance = isSwarmTroop(cardName) ? 5.0 : 3.0;
        double bestDistance = maxDistance;
        
        for (Troop troop : existingTroops) {
            if (alreadyMatched.contains(troop)) continue;
            if (troop.isPlayerSide() != isPlayerSide) continue;
            if (troop.getBaseCard() == null) continue;
            if (!troop.getBaseCard().getName().equals(cardName)) continue;
     ...
     * Uses smart updates to avoid flickering - updates existing buildings instead of recreating.
    ...
    private void applyBuildingState(String buildingData) {
        ...
        // Remove buildings that weren't in the host's state (they were destroyed on host)
     ...
     */
    private Building findMatchingBuilding(java.util.List<Building> existingBuildings, java.util.Set<Building> alreadyMatched,
            String cardName, boolean isPlayerSide, int targetX, int targetY) {
        for (Building building : existingBuildings) {
            if (alreadyMatched.contains(building)) continue;
            if (building.isPlayerSide() != isPlayerSide) continue;
            
            // Match by card name if available
            String buildingCardName = building.getCardName();
            if (buildingCardName != null && !buildingCardName.equals(cardName)) continue;
