...
public class NetworkBattleController implements GameEventListener {
    ...
    private void handleOpponentCardPlaced(NetworkMessage message) {
        ...
        System.out.println("[NetworkBattle] Spawning opponent's " + cardName + " at (" + x + ", " + mirroredY
                + ") [original y=" + y + "]");

        // Spawn the card for the opponent (isPlayer=false means enemy from our
        // perspective)
        ...
        if (selectedIndex != -1) {
            ...
            if (tileX >= 0 && tileX < Arena.WIDTH && tileY >= 0 && tileY < Arena.HEIGHT) {
                ...
                if (gameState.placeCard(true, selectedIndex, tileX, tileY)) {
                    ...
                    if (card != null) {
                        System.out.println("[NetworkBattle] YOU placed " + card.getName() + " at (" + tileX + ", "
                                + tileY + ") - sending to opponent");
