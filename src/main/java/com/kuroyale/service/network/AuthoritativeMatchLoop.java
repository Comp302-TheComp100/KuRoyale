 ...
 * 
 * This is the single source of truth for the match. It runs independently of
 * network
...
public class AuthoritativeMatchLoop {
     ...
     * The loop will run at a fixed tick rate, completely independent of network
     * conditions.
     */
    public void start() {
        ...
        // This NEVER blocks - it runs every TICK_INTERVAL_MS regardless of network
        // state
    ...
    private void applyInput(PlayerInput input, boolean isPlayer1) {
        if (input.getType() == PlayerInput.InputType.CARD_DEPLOY) {
            ...
            // Note: For player 2, coordinates are in their local space and need to be
            // mirrored
            int x = input.getX();
            int y = input.getY();
