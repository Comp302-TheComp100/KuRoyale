...
import com.kuroyale.model.core.enums.NetworkMessageType;
 ...
 * Implements the network protocol format:
...
public class NetworkMessage implements Serializable {
     ...
     * 
     * @param protocolString The string in format
     *                       MESSAGE_TYPE§§§player_id§§§data§§§timestamp
    ...
    /**
     * Creates an arena layout message to sync the host's arena design to the
     * client.
     * Format:
     * name#bridge1X,bridge1Y:bridge2X,bridge2Y:...#princess1X,princess1Y:princess2X,princess2Y#kingX,kingY
     */
     ...
     * Format:
     * troopId,cardName,x,y,health,isPlayer:troopId,cardName,x,y,health,isPlayer:...
     */
     ...
     * Format:
     * towerType,isPlayerSide,currentHealth,maxHealth,isAlive;towerType,isPlayerSide,...
     */
    ...
    /**
     * Section delimiter for FULL_STATE_SYNC - must be different from entity
     * delimiter (|)
     */
     ...
     * Format:
     * TROOPS#troops_data@@BUILDINGS#buildings_data@@GAME#gameTime,pElixir,bElixir,pScore,bScore,doubleElixir,gameOver
     * Note: Uses @@ as section delimiter to avoid collision with | used between
     * entities
     */
     ...
     * reason: description of why (e.g., "King Tower destroyed", "More towers",
     * "Lower HP tower")
     */
    ...
    public String[] parseCardPlacement() {
        if (type != NetworkMessageType.CARD_PLACED)
            return null;
    ...
    public String[] parsePlayerInfo() {
        if (type != NetworkMessageType.PLAYER_INFO)
            return null;
    ...
    public ArenaLayout parseArenaLayout() {
        if (type != NetworkMessageType.ARENA_LAYOUT)
            return null;
     ...
     * 
     * @return double array with [gameTime, playerElixir, botElixir, playerScore,
     *         botScore, isDoubleElixir (1.0 or 0.0)]
     */
    public double[] parseGameStateSync() {
        if (type != NetworkMessageType.GAME_STATE_SYNC)
            return null;
        try {
            String[] parts = data.split(";");
            if (parts.length < 6)
    ...
    public int[] parseScoreSync() {
        if (type != NetworkMessageType.SCORE_SYNC)
            return null;
        try {
            String[] parts = data.split(";");
            if (parts.length < 2)
    ...
    public String[] parseGameOver() {
        if (type != NetworkMessageType.GAME_OVER)
            return null;
        String[] parts = data.split(";", 2);
        return parts.length >= 2 ? parts : null;
    }
    ...
    public String getTowerSyncData() {
        if (type != NetworkMessageType.TOWER_SYNC)
            return null;
        return data;
    }
